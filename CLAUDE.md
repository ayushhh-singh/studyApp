...
- Session 2 (2026-07-05): full database schema.
  ...
  - Edge-case audit (live-tested): fixed one latent bug — `answer_submissions.question_id` was `ON DELETE SET NULL` which collided with the `has_prompt` CHECK (deleting a referenced question failed with a cryptic 23514); now `ON DELETE RESTRICT` (`0016`). Known/accepted by design: publish gate is stem-only (MCQ can publish without options/correct_key; `tests`/`current_affairs` have no gate), `*_i18n` shape isn't DB-validated (zod at API layer), and the public anon key currently has full RW under permissive RLS — all revisited in Session 15.
