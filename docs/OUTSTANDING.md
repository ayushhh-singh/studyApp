...
| A4 | **CSAT questions mostly held despite having keys** — blind-resolve is unreliable on comprehension/reasoning items, so many disagree→held even with a valid key. | Session 27.5 | 🟠 | Consider a **trust-the-official-key publish path for CSAT** that skips the blind-resolve requirement. Real limitation, not a bug. |
...
3. **Content depth (🟠, ongoing):** A1 (finish chapter rollout), A3/A4 (2021 GS-I key + CSAT publish path), A2 (Hindi overlay — large, low-urgency).
