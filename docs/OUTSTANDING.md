...
| A4-followups | **CSAT bank still has questions live on non-official keys** the new gate would hold. The additive-only re-gate (per instruction) left them live: (1) CSAT 2019/2020/2021 (~78) published on **unverified blind-proposed** keys; (2) CSAT 2022/2023 (~61) on coaching keys the blind solve **disagreed** with. A strict `pnpm ingest:regate --paper PRE_CSAT --apply` (no `--publish-only`) would hold these (CSAT 378→239). Also GS1: 2021 (−50, the A3 misaligned key), 2025 (−44 coaching-disagreed), null-year (−17) would be held by a full re-gate. | Session C (2026-07-13) | 🟠 | Deliberately deferred — un-publishing ~243 live questions is destructive + outside the "trust official keys" ask. Apply per-paper when ready; or apply the pending official/coaching CSAT keymaps first so more CSAT can pass the gate legitimately. |
