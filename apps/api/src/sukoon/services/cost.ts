/**
 * Sukoon admin cost dashboard (Session 13 hardening) — per-model daily spend,
 * summed from the shared llm_calls ledger (written by lib/anthropic.ts on every
 * model call). Sukoon's calls all use a `sukoon_` purpose prefix, so
 * `purpose LIKE 'sukoon_%'` isolates them cleanly. We sum the STORED cost_usd
 * (what was actually recorded at call time, batch discount already applied) —
 * the honest ledger figure, not a re-priced estimate.
 *
 * Admin-only (mounted behind requireAdmin in routes/admin.ts).
 */
import type { SukoonCostSummary, SukoonCostRow, SukoonCostDay } from "@neev/shared";
