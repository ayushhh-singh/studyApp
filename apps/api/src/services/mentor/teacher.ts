...
export async function generateQuickCheck(opts: {
   ...
   *
   * REQUIRED, deliberately — never give this a default. A defaulted trailing
   * parameter lets a caller silently keep the old behaviour (this repo's M24
   * lesson), which here would mean a second exam's user getting a quick-check
   * written in the default exam's framing, with nothing at the call site to
   * grep for. The one caller (`services/mentor/index.ts`) already holds the
   * real value as `plan.examCode`.
   */
  examCode: string;
