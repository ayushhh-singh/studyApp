...
async function loadWorkbook(month: string, examCode: string): Promise<MagazineMcq[]> {
  ...
  //
  // ⚑ THIS COMMENT USED TO SAY generated CA questions "leave exam_code at the
  // column default ('uppsc'), so this can only ever match uppsc rows". That is
  // STALE — measured 2026-08-08 across every CA question generated by a real
  // uppsc AND a real upsc run: 17/17 carry the exam of their own syllabus node,
  // 0 mismatches. The filter genuinely works per exam now. A second exam's
  // workbook can still read 0, but for the honest reason that CA MCQs are
  // inserted review-gated (`needs_review`) and this selects `approved`.
...
async function loadModelQuestions(month: string, examCode: string): Promise<MagazineModelQuestion[]> {
  ...
  // uppsc-only as an earlier version of this comment claimed.
  const { data, error } = await supabase()
    .from("questions")
