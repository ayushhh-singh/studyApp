...
export const queryKeys = {
  ...
  attempts: (page: number, paper?: string) => ["attempts", "list", page, paper ?? null] as const,
  ...
  reviewCounts: (exam: string) => ["admin", "review", "counts", exam] as const,
  reviewQueue: (tab: string, page: number, exam: string) => ["admin", "review", tab, page, exam] as const,
  caHighConfidenceCount: (exam: string) => ["admin", "review", "current-affairs", "high-confidence-count", exam] as const,
  ...
  dailyQuizArchive: (page: number, paper?: string) => ["daily-quiz", "archive", page, paper ?? null] as const,
  ...
  // a toast seen invalidates this too — which is right, since dismissing a toast
  // is the moment a freshly-earned badge should appear in the case. The case
  // itself is seen-independent: it returns the whole catalogue, earned or not,
  // unlike the unseen-only toast feed.
  badgeCase: () => ["milestones", "case"] as const,
  ...
  adminUserList: (page: number, query: string) => ["admin", "users", "list", page, query || "all"] as const,
